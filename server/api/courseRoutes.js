...
courseRoutes.patch(
  ...
  async (req, res) => {
    const courseId = req.params.courseId;
    const { course_status } = req.body; // Extract new status from request body

    console.log(courseId, course_status);
    const db = req.db;

    if (course_status === null) {
      res.status(400).json({ error: "Course status is required." });
      return;
    }

    try {
      await db.toggleCourseStatus(courseId, course_status); // Assume this function updates the status
